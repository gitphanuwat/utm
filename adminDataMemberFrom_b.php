<?php
	...
	if($_GET["action"]=="getFromMember"){
		...
		echo " <form action='adminDataMemberFrom.php?action=insert' id='myform' method='post' target='upload_target' onsubmit='clickSave();' >";
            ...
            echo "<div class='box-body'>";
            	...
            	echo "</div>";
            		echo "<div class='form-group'>";
						...
						if($_SESSION["DUR_USER_STATE"]=="MANAGER" or $_SESSION["DUR_USER_STATE"]=="USER"){
							$ugroup=$_SESSION["DUR_USER_GROUP"];
							echo "<select class='form-control' name='cboPosition'>";
								$sql="select idgroup , groupname from tb_group where idgroup=$ugroup";
								//$sql=$sql . " where keyman=1";
								$result=mysqli_query($connect,$sql);
								//echo "<option value=0> ==เลือกกลุ่ม==</option>";
								$row=mysqli_fetch_array($result);
					...
					echo "</div>";
            		echo "<div class='form-group'>";
						echo "<label >สิทธิ์การใช้ระบบ</label>";
            ...
            echo "<div class='box-footer'>";
            	echo "<button type='button' class='btn btn-primary' id='checkSave'>Save</button>&nbsp;";
	...
	if($_GET["action"]=="insert"){
		...
		if($Prefix=="" || $txtFirstname=="" || $txtLastname=="" || $cboCourse==0){
			$msgerror=1;
		}else{
			if($iduser !=""){
			...
			$result=mysqli_query($connect,$sql);
			$msgsuccess=1;
					echo "<script language=\"javascript\">window.location.href = 'adminMember.php'</script>";
		}
	}
